# Context
## 前言
本文主要介绍Golang并发编程中常用到一种编程模式：context。从为什么需要context出发，深入了解context的实现原理，以及了解如何使用context。
## 为什么需要context
...
* `Err` 如果`Done`返回的`channel`没有关闭，将返回`nil`;如果`Done`返回的`channel`已经关闭，将返回非空的值表示任务结束的原因。如果是`context`被取消，Err将返回`Canceled`；如果是`context`超时，`Err`将返回`DeadlineExceeded`。
...
```
跟`valueCtx`类似，`cancelCtx`中也有一个`context`变量作为父节点；变量`done`表示一个`channel`，用来表示传递关闭信号；`children`表示一个map，存储了当前`context`节点下的子节点；`err`用于存储错误信息表示任务结束的原因。
...
```
1. 如果父节点`parent`有过期时间并且过期时间早于给定时间d，那么新建的子节点`context`无需设置过期时间，使用`WithCancel`创建一个可取消的`context`即可；
...
## 最佳实践
1. `context.Background`只能在最高级别使用，作为所有派生`context`的根
2. 如果不确定使用什么或将来将更新当前功能以使用上下文，则应使用`context.TODO`
3. `context`取消是建议性的，`functions`可能需要一些时间才能清理并退出（即取消`context`后并不意味着`goroutine`会立刻回收）
4. `context.Value`应该很少使用，绝不应该用于传递可选参数。这会使API隐式出现，并可能引入错误。相反，此类值应作为参数传递。
5. 不要将`context`存储在结构体中，最好在函数中显式传递它们，最好将其作为第一个参数。
6. 永远不要传递`nil context`，相反，如果不确定要使用什么内容，请使用`TODO`。
7. `Context`结构体本身没有`cancel`方法，因为只有派生`context`的函数才应该取消它。
## 总结
...
## 参考资料
1. [标准库文档](https://golang.org/pkg/context/)
2. [官方博客](https://blog.golang.org/context)
3. [第三方博客](http://p.agnihotry.com/post/understanding_the_context_package_in_golang/)
